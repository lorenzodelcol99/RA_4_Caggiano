...
new_gfu = new_gfu.drop(columns=['Date'])

# ==========================================
# The Mapping Dictionary, to overcome different names in the different files, I've manainted the structure as it was originally
# ==========================================
aliases = {
...
def get_phase_b_column(df, country):
    """Search the column using the original name, then the aliases, then the US/USA variants"""
    # 1. Try the exact match
...
# ==========================================
# Inserting new data in the new global factor dataset
# ==========================================
for col in new_gfu.columns:
    ...
    if len(parts) == 2:
        ...
        # Understanding in which datafram to look trough
        target_df = None
        ...
        if target_df is not None:
            # this tryes to find the exact name of the column
            sd_col_name = get_phase_b_column(target_df, country)
            
            if sd_col_name:
                new_gfu[col] = target_df[sd_col_name]
            else:

...
randn('state',999);  
for gibbs=1:n_gibbs,
    dispX = ['Gibbs sampler, step % ', num2str((gibbs/n_gibbs)*100)]; disp(dispX);
    %fprintf('Gibbs sampler, step %',gibbs/n_gibbs*100);
...
%         else, % if Bsub(b) == 0
            ...
            for s=1:Bsub(b);           
                ...
                F_Xbs_sample = EM_DFM_SS(yy,Par);
                H{b}{s}=F_Xbs_sample.F;
                Lambda_H{b}{s}{1}=F_Xbs_sample.C;

% Preliminary EM analysis [F,G,H,FH,FX,GX,K_F,K_G,K_H]=do_EM_level4(bigX,Bsub)
[F_em,G_em,H_em,FH_em,FX_em,GX_em,K_F_em,K_G_em,K_H_em,Lambda_H_em,bigXmat,bigGmat,bigHmat]=do_EM_level4(bigZ_ns,Bsub);


paramsF.B = B; paramsF.q_G = q_G; paramsF.K_blocks = K_G; paramsF.q_F = q_F*ones(K_F,1); paramsF.K_facs = K_F; paramsF.l_F = l_F;

% Initialize factors as EM estimates plus random noise
T=rows(bigZ_ns{1}{1});
bigZ=bigZ_ns;
for b=1:B;
    g{b}=G_em{b}(:,1:K_G(b)) + noise_var*randn(T,K_G(b));
%     if Bsub(b) == 0,
%         bigZ{b}=standard_miss(cell2mat(bigZ_ns{b}),999.99);
% %         for k = 1:K_G(b),
...
%              bigZ{b}{s}=standard_miss(bigZ_ns{b}{s},999.99);
            h{b}{s}=H_em{b}{s}(:,1:K_H{b}(s)) + noise_var*randn(T,K_H{b}(s));     
%             for k = 1:K_H{b}(s),
...
for i = 1:K_F,                
    ...
    f(:,i) = f(:,i)*signlam(i);
    % LORENZO: Old --> for l = 1:l_F+1, transformed in line 65
    for l = 1:l_F(b)+1,
...
%             lambda_Hbs = ((hbs_lags'*hbs_lags)\(hbs_lags'*yy))';
             % Take laodings estimate from EM algorithm: lambda_Hbs
%             for l = 1:l_H(b)+1,
% %                 Lambda_H{b}{s}{l} = lambda_Hbs(:,(l-1)*K_H{b}(s)+1:l*K_H{b}(s));     
%           
%             end;
            Lambda_H= Lambda_H_em;
...
%             eZ{b}{s} = compute_resids(yy,h{b}{s},cell2mat(Lambda_H{b}{s}),paramsH);
              eZ{b}{s} = randn(T,cols(bigZ_ns{b}{s}));
            paramsZ.q_F = q_Z(b)*ones(size(yy,2),1); paramsZ.K_facs = size(yy,2); paramsZ.l_F = l_H(b)*ones(size(yy,2),1); 

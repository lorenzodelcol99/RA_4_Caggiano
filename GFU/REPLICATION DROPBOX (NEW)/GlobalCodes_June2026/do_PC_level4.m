...
for b=1:B,
    ...
    bigXmat=[bigXmat Xb]; 
    [ic_Gb,chat_Gb,GX_b,eigval_Gb]=nbplog(Xb,k_max,2,0);
    if isempty(K_G), K_G(b) = min(max(ic_Gb,1),3); end;
    [ehat_Gb,GX{b},lambda_Gb,ss_Gb]=pc_T(standard(Xb),K_G(b));
    for s = 1:Bsub(b), 
        Zbs = bigX{b}{s};
        [ic_Hbs,chat_Hbs,HX_bs,eigval_Hbs]=nbplog(Zbs,min(size(Zbs,2),k_max),2,0);
...
% Extract F from H's
[ic_FH,chat_FH,fhat_FH,eigval_FH]=nbplog(bigHmat,min(k_max,size(bigHmat,2)),2,0);
[ehat2,FH,lambda_FH,ss3]=pc_T(bigHmat,ic_FH);

% Extract F from X's
[ic_FX,chat_FX,fhat_FX,eigval_FX]=nbplog(bigXmat,k_max,2,0);
if isempty(K_F), K_F = min(max(ic_FX,k_max),3); end;
[ehat3,FX,lambda_FX,ss5]=pc_T(standard(bigXmat),K_F);

% Extract F from G's
[ic_F,chat_F,fhat_F,eigval_F]=nbplog(bigGmat,min(k_max,size(bigGmat,2)),2,0);

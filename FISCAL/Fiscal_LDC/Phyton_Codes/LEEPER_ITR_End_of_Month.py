...
########### ALTERNATIVE CODE: USIGN ONLY LEEPER MUNISDATA2 DATA FOR THE GOV BONDS YIELDS (FROM APRIL 1953 TO 2008) AND THE FRED DATA FOR THE PERIOD FROM FEBRUARY 2009 TO JULY 2024.
...
Leeper_Gov_Bond_End_of_Month_df = MunisData2.iloc[:, [0, 2, 5 ]].set_axis(Gov_Bond_End_of_Month_df.columns, axis=1)

Gov_Bond_End_of_Month_df = Gov_Bond_End_of_Month_df[Gov_Bond_End_of_Month_df['observation_date'] >= '2009-01-30'].reset_index(drop=True) # keeping only the observations from 2009 onwards

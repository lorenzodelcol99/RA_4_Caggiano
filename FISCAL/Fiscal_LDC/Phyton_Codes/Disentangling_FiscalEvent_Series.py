...
for idx in non_zero_indices:
    # Salta le righe già processate correttamente
    if df.at[idx, 'LLM_Explanation'] not in ["", "Local Processing Error", None]:
        continue 
    ...
    # Salva su disco dopo OGNI riga (così puoi interrompere quando vuoi)
    df.to_csv(OUTPUT_FILE, index=False)
    
    if non_zero_indices.index(idx) % 10 == 0:
        print(f"Progress: {non_zero_indices.index(idx)} / {len(non_zero_indices)} (Saved)")
